  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 flex flex-col">
      <header className="text-center mb-8">
        ...
        <p className="text-gray-600 mt-2">Calculator, sequences, word problems & visualizations</p>
      </header>

      <main className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
        <section className="p-6 bg-white rounded-2xl shadow">
          <h2 className="text-2xl mb-4">Calculator</h2>
          <div className="flex gap-2 mb-3">
            <input
              className="border rounded px-3 py-2 flex-1"
              placeholder="Enter expression (e.g., 2+3*4 or x^2+2x+1)"
              value={expression}
          ...
          {calcData.length > 0 && (
            <LineChart width={300} height={200} data={calcData}>
        ...
        <section className="p-6 bg-white rounded-2xl shadow">
          ...
          {sequence.length > 0 && (
            <>
              ...
              <LineChart width={300} height={200} data={sequence.map((val, i) => ({ index: i+1, val }))}>
                <Line type="monotone" dataKey="val" stroke="#16a34a" />
                <CartesianGrid stroke="#ccc" />
                <XAxis dataKey="index" />
                <YAxis />
                <Tooltip />
